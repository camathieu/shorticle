...
public class ShorticleServer extends Verticle {
    Logger log;

    @Override
    public void start() {
        log = container.logger();
        log.info("Shorticle Server is alive");
        container.deployVerticle("gg.root.Shorticle", container.config(), 8, new AsyncResultHandler<String>() {
            public void handle(AsyncResult<String> asyncResult) {
                if (asyncResult.succeeded()) {
                    log.debug("The verticle ShorticleServer has been deployed, deployment ID is " + asyncResult.result());
                    startHttpServer();
                } else {
    ...
    void startHttpServer() {
        ...
        inWhitelist.add(new JsonObject().putString("address", "shorticle.get"));
        inWhitelist.add(new JsonObject().putString("address", "shorticle.set"));
